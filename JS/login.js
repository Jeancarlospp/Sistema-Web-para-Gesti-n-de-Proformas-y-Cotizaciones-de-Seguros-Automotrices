...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!loginForm) {
        console.error("Error crítico: El formulario con ID 'login-form' no fue encontrado en la página.");
        return;
    }
    ...
    loginForm.addEventListener('submit', async (event) => {
        ...
        // --- 3. LLAMADA A LA API (BACKEND) CON MANEJO DE ERRORES ---
         try {
            ...
            displayError('No se pudo conectar con el servidor. Verifique la ruta y que el servidor esté activo.');
            setButtonState('default');
        }
    });
