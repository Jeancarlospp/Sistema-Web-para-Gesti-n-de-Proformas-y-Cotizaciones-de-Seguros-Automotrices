...
function populateProfileForms(userData) {
    ...
    document.getElementById('profile-card-name').textContent = userData.nombre || '';
    document.getElementById('profile-card-role').textContent = userData.rol || '';
    document.getElementById('profile-card-email').textContent = userData.correo || '';
...
async function fetchCurrentUser() {
    try {
        ...
        alert("No se pudieron cargar los datos de tu perfil. Intenta recargar la página.");
    }
}
    ...
    formEditProfile.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                document.getElementById('profile-name').textContent = profileData.nombre;
                document.getElementById('profile-greeting-name').textContent = profileData.nombre.split(' ')[0];
