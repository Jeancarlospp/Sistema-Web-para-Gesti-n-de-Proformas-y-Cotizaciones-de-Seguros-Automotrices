...
function renderUsersTable(users, tableBody) {
    ...
    if (!users || users.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-muted">No se encontraron usuarios que coincidan con los criterios.</td></tr>';
        return;
    }
    users.forEach(user => {
        const estadoBadge = user.estado === 'activo' ? '<span class="badge bg-success">Activo</span>' : '<span class="badge bg-secondary">Inactivo</span>';
        ...
        row.innerHTML = `
            ...
            <td>${estadoBadge}</td>
            <td>
                <button class="btn btn-sm btn-info btn-edit" data-id="${user.id_usuario}" title="Editar Usuario"><i class="bi bi-pencil"></i></button>
                <button class="btn btn-sm ${actionButtonClass} btn-toggle-status ms-1" data-id="${user.id_usuario}" data-new-status="${user.estado === 'activo' ? 'inactivo' : 'activo'}" title="${actionButtonText} Usuario">${actionButtonText}</button>
            </td>`;
...
function renderPaginationControls(container) {
    ...
    buttonsHtml += `<li class="page-item ${tableState.currentPage === 1 ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${tableState.currentPage - 1}">Anterior</a></li>`;
    for (let i = 1; i <= totalPages; i++) {
        buttonsHtml += `<li class="page-item ${tableState.currentPage === i ? 'active' : ''}"><a class="page-link" href="#" data-page="${i}">${i}</a></li>`;
    }
    buttonsHtml += `<li class="page-item ${tableState.currentPage === totalPages ? 'disabled' : ''}"><a class="page-link" href="#" data-page="${tableState.currentPage + 1}">Siguiente</a></li>`;
    buttonsHtml += '</ul>';
...
function updatePaginationInfo(element) {
    if (tableState.totalRecords === 0) { element.textContent = 'No hay registros'; return; }
...
async function fetchAndRenderUsers() {
    ...
    try {
        const response = await fetch(`../php/usuarios_api.php?${params.toString()}`);
...
async function populateRolesSelects(...elements) {
    try {
        ...
        elements.forEach(el => { if (el) el.innerHTML = '<option value="">Error</option>'; });
    }
}
...
async function openEditModal(userId) {
    try {
        ...
        if (!response.ok) throw new Error('No se pudo obtener la info del usuario.');
        const user = await response.json();
        if (!user || user.error) throw new Error(user.error || 'Usuario no encontrado.');
...
export function loadGestionUsuarios() {
            ...
            if (confirm(`¿Seguro que deseas ${newStatus} este usuario?`)) {
                try {
                    const response = await fetch('../php/usuarios_api.php', {
                        ...
                        body: JSON.stringify({ action: 'update_estado', id: userId, estado: newStatus })
                    });
                    const result = await response.json();
                    if (result.success) await fetchAndRenderUsers();
                    else throw new Error(result.message);
                } catch (error) { alert('Error al cambiar estado: ' + error.message); }
            }
        }
    });
    ...
    btnSaveUser.addEventListener('click', async () => {
        if (!addUserForm.checkValidity()) { addUserForm.reportValidity(); return; }
        const newUser = {
            action: 'create_user',
        ...
        try {
            const response = await fetch('../php/usuarios_api.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newUser) });
            const result = await response.json();
            if (result.success) {
    ...
    btnUpdateUser.addEventListener('click', async () => {
        if (!editUserForm.checkValidity()) { editUserForm.reportValidity(); return; }
        const updatedUser = {
            action: 'update_user',
        ...
        try {
            const response = await fetch('../php/usuarios_api.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedUser) });
            const result = await response.json();
            if (result.success) {
