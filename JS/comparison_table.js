...
export async function buildComparisonTable(
  ...
  try {
    ...
    const response = await fetch(
      `../php/productos_api.php?ids=${idsQueryString}`
    );
    if (!response.ok)
    ...
    if (!products || products.length === 0) {
      setTableState(
        "No se encontraron detalles para los planes seleccionados.",
        tableHead,
        tableBody,
        tableFoot
      );
    ...
    let descriptionRow =
      '<tr><td class="text-start"><strong>Descripción</strong></td>';
    products.forEach((product) => {
      descriptionRow += `<td class="text-start"><small>${
        product.Pro_descripcion || "N/A"
      }</small></td>`;
    });
    ...
    let monthsRow =
      '<tr><td class="text-start"><strong>Meses de Cobertura</strong></td>';
    products.forEach((product) => {
      // CORRECCIÓN: Se usa `Pro_mesesCobertura` que coincide con el SQL.
      monthsRow += `<td class="fw-bold">${
        product.Pro_mesesCobertura || "N/A"
      } meses</td>`;
    });
    ...
    Object.keys(featureLabels).forEach((featureKey) => {
      ...
      products.forEach((product) => {
        ...
        if (
          (featureKey === "Pro_roboTotal" ||
            featureKey === "Pro_autoReemplazo") &&
          value === "si"
        ) {
          ...
          cellContent = `<i class="bi bi-check-circle-fill text-success fs-4"></i><br><small>Hasta $${parseFloat(
            value
          ).toLocaleString("es-EC")}</small>`;
        } else if (
          typeof value === "string" &&
          value &&
    ...
    let footRow =
      '<tr class="table-light"><td class="text-start"><strong>Precio Mensual</strong></td>';
    products.forEach((product) => {
      footRow += `<td><h4>$${parseFloat(product.Pro_precioMensual).toFixed(
        2
      )}</h4></td>`;
    });
